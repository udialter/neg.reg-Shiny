regpd <- function(data=NULL, formula=NULL, predictor=NULL, #input for full dataset
                    ...
                    plots = TRUE, saveplots = FALSE, seed=NA,...) # optional input for both
{
  ...
  if(!is.null(formula)){ # when dataset is entered
    ...
    ifelse(length(vars)==2, formula <- newform, formula <- paste(newform,vars[3:length(vars)],sep = "+"))
    ...
    for (i in 1:nboot) {
      ...
      temp.model <- lm(formula=formula, data=temp.data)
